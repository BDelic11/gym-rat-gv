...
export default function FoodClient({
  ...
  // ---- API helpers ----
  async function apiLoadMeals() {
    const res = await fetch("/api/meals", { cache: "no-store" });
    const data = await res.json();
    if (!data?.success) throw new Error(data?.error || "Failed to load meals");
    setMeals(data.meals || { breakfast: [], lunch: [], dinner: [], snack: [] });
  }
    ...
    <>
      <div className="space-y-6">
        <MealSection
          ...
          onBrowseTemplates={() => setBrowseType("BREAKFAST")}
        />
      ...
      {browseType && (
        <TemplateBrowserModal
          isOpen={!!browseType}
          mealType={browseType as MealType}
