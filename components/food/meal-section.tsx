...
export function MealSection({ type, title, items, onAddFood, onRemoveItem }: MealSectionProps) {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => onAddFood(type)} className="gap-2">
                ...
                <div key={item.id} className="flex items-center justify-between p-2 bg-muted/50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">
                      {item.calories} cal • P: {item.protein.toFixed(1)}g • C: {item.carbs.toFixed(1)}g • F:{" "}
                      {item.fat.toFixed(1)}g
                    </div>
                  </div>
                  {onRemoveItem && (
                    <Button variant="ghost" size="sm" onClick={() => onRemoveItem(item.id)} className="h-8 w-8 p-0">
